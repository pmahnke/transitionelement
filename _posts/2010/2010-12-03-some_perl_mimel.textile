...
h3. An email with no images, just empty boxes
...
h3. An email with no images, empty boxes and a message asking you if you want to download them.
...
# parses the HTML looking for &lt;img tags
# if it finds one, it checks if the file is local or remote
## if it is remote, it _wget_ the file and makes it a local one (you have to delete these later)
# then it turns the source of the image to the correct new one &lt;img src="cid:
# finally, it attaches the file, using "base64":http://en.wikipedia.org/wiki/Base64
